...
namespace Google.Api.Ads.AdWords.Examples.CSharp.v201008 {
  ...
  /// </summary>
  class HandlePartialFailures : SampleBase {
    /// <summary>
    ...
    public static void Main(string[] args) {
      SampleBase codeExample = new HandlePartialFailures();
    ...
    public override void Run(AdWordsUser user) {
      ...
      try {
        ...
        if (result != null && result.value != null) {
          foreach (AdGroupCriterion adGroupCriterionResult in result.value) {
            if (adGroupCriterionResult.criterion != null) {
              Console.WriteLine("Ad group criterion with ad group id '{0}', and criterion " +
                  "id '{1}', and keyword '{2}' was added.\n", adGroupCriterionResult.adGroupId,
        ...
        if (result != null && result.partialFailureErrors != null) {
          foreach (ApiError apiError in result.partialFailureErrors) {
            ...
            if (operationIndex != -1) {
              ...
              Console.WriteLine("Ad group criterion with ad group id '{0}' and keyword '{1}' "
        ...
        Console.WriteLine("Failed to add keyword(s) in partial failure mode. Exception says " +
            "\"{0}\"", e.Message);
      }
    }
  }
