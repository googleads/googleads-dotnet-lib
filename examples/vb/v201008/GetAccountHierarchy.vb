  ...
  Class GetAccountHierarchy
    ...
    Public Overrides Sub Run(ByVal user As AdWordsUser)
      ...
      Dim servicedAccountService As ServicedAccountService = user.GetService( _
          AdWordsService.v201008.ServicedAccountService)

      Dim selector As New ServicedAccountSelector
      selector.serviceTypes = New ServiceType() {ServiceType.API_ONLY, ServiceType.UI_AND_API}
      ...
      Try
        ...
        If ((Not graph Is Nothing) AndAlso (Not graph.accounts Is Nothing)) Then
          ...
          For Each link As Link In graph.links
            Console.WriteLine("There is a {0} link of type {1} from {2:###-###-####} to " & _
                "{3:###-###-####}", link.typeOfLink, link.serviceType, link.managerId.id, _
                link.clientId.id)
          Next
        End If
