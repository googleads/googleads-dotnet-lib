  ...
  public class CreateCustomTargetingKeysAndValues : SampleBase {
    ...
    public void Run(DfpUser user) {
      using (CustomTargetingService customTargetingService =
          (CustomTargetingService) user.GetService(DfpService.v201708.CustomTargetingService)) {
        ...
        try {
          ...
          if (keys != null) {
            foreach (CustomTargetingKey key in keys) {
              Console.WriteLine("A custom targeting key with ID \"{0}\", name \"{1}\", and " +
                  "display name \"{2}\" was created.", key.id, key.name, key.displayName);
            }
          } else {
          ...
          if (returnValues != null) {
            foreach (CustomTargetingValue value in returnValues) {
              Console.WriteLine("A custom targeting value with ID \"{0}\", belonging to key " +
                  "with ID \"{1}\", name \"{2}\", and display name \"{3}\" was created.", value.id,
