...
namespace Google.Api.Ads.Dfp.Examples.CSharp.v201711 {
  ...
  public class CreateProgrammaticProductTemplates : SampleBase {
    ...
    public override string Description {
      get {
        return "This example creates a programmatic product template. Your network must have "
            + "sales management enabled to run this example. To publish the created product "
            + "template to Marketplace, you must create a ProductTemplateBaseRate with a "
            + "Marketplace rate card.";
      }
    }
    ...
    public static void Main() {
      CreateProgrammaticProductTemplates codeExample = new CreateProgrammaticProductTemplates();
    ...
    public void Run(DfpUser user) {
      using (ProductTemplateService productTemplateService =
          (ProductTemplateService) user.GetService(DfpService.v201711.ProductTemplateService))

      using (NetworkService networkService =
          (NetworkService) user.GetService(DfpService.v201711.NetworkService)) {
