  ...
  class ApproveOrder : SampleBase {
    ...
    public override void Run(DfpUser user) {
      ...
      OrderService orderService =
          (OrderService) user.GetService(DfpService.v201108.OrderService);

      // Create Statement text to select all draft orders.
      string statementText = "WHERE status IN (:status1, :status2) LIMIT 500";
      Statement statement = new StatementBuilder("").AddValue("status1",
          OrderStatus.DRAFT.ToString()).AddValue("status2", OrderStatus.PENDING_APPROVAL.
          ToString()).ToStatement();
