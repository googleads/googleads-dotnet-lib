...
namespace Google.Api.Ads.Dfp.Examples.v201108 {
  /// <summary>
  /// This code example updates the descriptions of all labels that are
  /// competitive exclusion by updating its description up to the first 500.
  /// To determine which labels exist, run GetAllLabels.cs. This feature is only
  ...
  class UpdateLabels : SampleBase {
    ...
    public override string Description {
      get {
        return "This code example updates the descriptions of all labels that are competitive " +
    ...
    public override void Run(DfpUser user) {
      ...
      LabelService labelService =
          (LabelService) user.GetService(DfpService.v201108.LabelService);

      // Create a statement to only select labels that are competitive
      // exclusion.
      Statement filterStatement = new StatementBuilder("WHERE type = :type and isActive = true " +
          "LIMIT 500").AddValue("type", LabelType.COMPETITIVE_EXCLUSION.ToString()).ToStatement();
