...
namespace Google.Api.Ads.Dfp.Examples.v201203 {
  /// <summary>
  /// This code example gets all teams that you belong to. The statement
  ...
  class GetTeamsByStatement : SampleBase {
    ...
    public override string Description {
      get {
        return "This code example gets all teams that you belong to. The statement retrieves up " +
            "to the maximum page size limit of 500. To create teams, run CreateTeams.cs.";
      }
    }
    ...
    public override void Run(DfpUser user) {
      ...
      try {
        ...
        long[] teamIds = userService.getCurrentUser().teamIds;

        if (teamIds != null) {
          StringBuilder builder = new StringBuilder();
          foreach (long teamId in teamIds) {
            builder.AppendFormat("{0}, ", teamId);
          }
          // Create a statement to only select teams by their IDs.
          Statement filterStatement = new StatementBuilder("WHERE id IN (" +
              builder.ToString().TrimEnd(' ', ',') + ") LIMIT 500").ToStatement();
          ...
          Console.WriteLine("No teams found for current user.");
        }
      } catch (Exception ex) {
