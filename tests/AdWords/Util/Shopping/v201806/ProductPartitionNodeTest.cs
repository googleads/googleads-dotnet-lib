...
namespace Google.Api.Ads.AdWords.Tests.Util.Shopping.v201806
{
    ...
    public class ProductPartitionNodeTest
    {
        ...
        public void TestRootNodeBasicFunctionality()
        {
            ...
            Assert.False(rootNode.IsSubdivision,
                "New node should not be a subdivision by default.");
            Assert.True(rootNode.IsBiddableUnit,
        ...
        {
            Assert.Throws<InvalidOperationException>(delegate() { rootNode.AsExcludedUnit(); });
        }
        ...
        {
            Assert.True(rootNode.IsUnit, "root should be a unit by default.");
            Assert.Throws<ArgumentException>(delegate() { rootNode.CpcBid = -1; });
        }
