...
namespace Google.Api.Ads.Common.Tests.Util {
  ...
  /// <summary>
  /// UnitTests for <see cref="UrlEncodedBodyFormatter"/> class.
  /// </summary>
  [TestFixture]
  public class UrlEncodedBodyFormatterTests {
    ...
    private readonly string BODY = string.Format("{0}={1}&{2}={3}&{4}={5}",
        HttpUtility.UrlEncode(KEY1),
        HttpUtility.UrlEncode(VALUE1),
        HttpUtility.UrlEncode(KEY2),
        HttpUtility.UrlEncode(VALUE2),
        HttpUtility.UrlEncode(KEY3),
        HttpUtility.UrlEncode(VALUE3));

    private readonly string FORMATTED_BODY = string.Format("{0}={1}&{2}={3}&{4}={5}",
        HttpUtility.UrlEncode(KEY1),
        KeyValueMessageFormatter.MASK_PATTERN,
        HttpUtility.UrlEncode(KEY2),
        KeyValueMessageFormatter.MASK_PATTERN,
        HttpUtility.UrlEncode(KEY3),
        HttpUtility.UrlEncode(VALUE3));
