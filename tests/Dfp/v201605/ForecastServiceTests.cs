...
namespace Google.Api.Ads.Dfp.Tests.v201605 {
  /// <summary>
  ...
  public class ForecastServiceTests : BaseTests {
    ...
    public void Init() {
      ...
      forecastService = (ForecastService)user.GetService(DfpService.v201605.ForecastService);
    ...
    /// </summary>
    [Test]
    public void TestGetAvailabilityForecastById() {
      AvailabilityForecast forecast = null;
      Assert.DoesNotThrow(delegate() {
        forecast = forecastService.getAvailabilityForecastById(lineItemId,
            new AvailabilityForecastOptions());
      });
      Assert.NotNull(forecast);
      Assert.AreEqual(forecast.orderId, orderId);
