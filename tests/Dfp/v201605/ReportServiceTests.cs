...
namespace Google.Api.Ads.Dfp.Tests.v201605 {
  ...
  public class ReportServiceTests : BaseTests {
    ...
    public void Init() {
      ...
      reportService = (ReportService) user.GetService(DfpService.v201605.ReportService);
    ...
    public void TestGetReportJobStatus() {
      Assert.DoesNotThrow(delegate() {
        reportService.getReportJobStatus(reportId);
      });
    }
    ...
    public void TestGetReportDownloadUrl() {
      ...
      Assert.DoesNotThrow(delegate() {
        url = reportService.getReportDownloadURL(reportId, ExportFormat.TSV_EXCEL);
      });
