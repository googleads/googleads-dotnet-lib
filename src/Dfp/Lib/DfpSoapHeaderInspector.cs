...
namespace Google.Api.Ads.Dfp.Lib {
  ...
  /// </summary>
  public class DfpSoapHeaderInspector : IClientMessageInspector {
    ...
    public DfpAppConfig Config { get; set; }

    void Validate() {
      if (Config == null) {
        // TODO (cseeley): Add a a message to DfpErrorMessages for this.
        throw new DfpApiException(null, "Config cannot be null");
      }
      if (RequestHeader == null) {
        // TODO (cseeley): Add a a message to DfpErrorMessages for this.
        throw new DfpApiException(null, "RequestHeader cannot be null");
      }
      if (string.IsNullOrWhiteSpace(Config.ApplicationName) || Config.ApplicationName.Contains(
          DfpAppConfig.DEFAULT_APPLICATION_NAME)) {
        throw new DfpApiException(null, DfpErrorMessages.RequireValidApplicationName);
      }
    }
