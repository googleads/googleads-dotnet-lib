...
namespace Google.Api.Ads.AdWords.Util.Reports
{
    ...
    public class ReportUtilities : AdsReportUtilities
    {
        ...
        /// </summary>
        internal const string DEFAULT_REPORT_VERSION = "v201809";
        ...
        /// </summary>
        private const string QUERY_REPORT_URL_FORMAT =
            "{0}/api/adwords/reportdownload/{1}?" + "__fmt={2}";
        ...
        public ReportUtilities(AdWordsUser user, string query, string format) : this(user,
            DEFAULT_REPORT_VERSION, query, format)
        {
        }
        ...
        public ReportUtilities(AdWordsUser user, IReportDefinition reportDefinition) : this(user,
            DEFAULT_REPORT_VERSION, reportDefinition)
        {
        }
        ...
        public ReportUtilities(AdWordsUser user, string reportVersion,
            IReportDefinition reportDefinition) : base(user)
        {
            this.reportVersion = reportVersion;
        ...
        protected override ReportResponse GetReport()
        {
            ...
            else
            {
                downloadUrl = string.Format(ADHOC_REPORT_URL_FORMAT, config.AdWordsApiServer,
                    reportVersion);
                postBody = "__rdxml=" +
                    HttpUtility.UrlEncode(ConvertDefinitionToXml(reportDefinition));
            }
        ...
        private ReportResponse DownloadReport(string downloadUrl, string postBody)
        {
            ...
            catch (WebException e)
            {
                ...
                logEntry.LogResponse(new ResponseInfo(e.Response, "")
                {
        ...
        {
            string xml = SerializationUtilities.SerializeAsXmlText(definition)
                .Replace("ReportDefinition", "reportDefinition");
