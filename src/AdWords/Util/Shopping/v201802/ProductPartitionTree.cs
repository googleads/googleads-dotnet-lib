...
namespace Google.Api.Ads.AdWords.Util.Shopping.v201802
{
    ...
    public class ProductPartitionTree
    {
            ...
            List<AdGroupCriterion> adGroupCriteria)
        {
            ...
            Dictionary<long, List<AdGroupCriterion>> parentIdMap =
                CreateParentIdMap(adGroupCriteria);
        ...
        private static Money GetBid(BiddableAdGroupCriterion biddableCriterion)
        {
            BiddingStrategyConfiguration biddingConfig =
                biddableCriterion.biddingStrategyConfiguration;
            ...
            ProductPartitionNode newNode, List<OperationPair> ops, TemporaryIdGenerator idGenerator)
        {
            ...
            switch (nodeDifference)
            {
                case NodeDifference.NEW_NODE:
                    ...
                    // No need to further process children. The ADD operations above will include
                    // operations for all children of newNode.
                    isProcessChildren = false;
                    break;

                case NodeDifference.REMOVED_NODE:
                    ops.Add(CreateRemoveOperation(originalNode));
                    // No need to further process children. The REMOVE operation above will perform
                    // a cascading delete of all children of newNode.
                    isProcessChildren = false;
                    break;
                ...
                case NodeDifference.BID_CHANGE:
                    // Ensure that the new node has the proper ID (this may have been lost if the
                    ...
                    // Process the children of newNode. The SET operation above will only handle
                    // changes made to newNode, not its children.
                ...
                default:
                    throw new InvalidOperationException(
                        "Unrecognized difference: " + nodeDifference);
            }
            ...
            if (isProcessChildren)
            {
                ...
                foreach (ProductPartitionNode newChild in newNode.Children)
                {
                    if (originalNode.HasChild(newChild.Dimension))
                    {
                        ...
                        AddMutateOperations(originalNode.GetChild(newChild.Dimension), newChild,
                            ops, idGenerator);
                    }
                    else
                    {
        ...
        private OperationPair CreateRemoveOperation(ProductPartitionNode node)
        {
            ...
            removeOp.operand =
                ProductPartitionNodeAdapter.CreateCriterionForRemove(node, adGroupId);
            ...
            ProductPartitionNode newNode, IEqualityComparer<ProductDimension> dimensionComparator)
        {
            ...
            else if (!dimensionComparator.Equals(originalNode.Dimension, newNode.Dimension))
            {
                throw new InvalidOperationException(string.Format(
                    ShoppingMessages.ProductDimensionMismatch, originalNode.Dimension,
                    newNode.Dimension));
            }
            ...
            else if (!originalNode.IsExcludedUnit && originalNode.IsUnit && newNode.IsUnit)
            {
                ...
                // left is the bid.
                nodeDifference = (originalNode.CpcBid != newNode.CpcBid)
                    ? NodeDifference.BID_CHANGE
                    : NodeDifference.NONE;
            }
            else
            {
