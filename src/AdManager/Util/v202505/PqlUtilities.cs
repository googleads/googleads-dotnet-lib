...
namespace Google.Api.Ads.AdManager.Util.v202505
{
    /// <summary>
    ...
    public class PqlUtilities
    {
        ...
        private static string GetTextValue(Object value)
        {
            ...
            if (value is Google.Api.Ads.AdManager.v202505.Date)
            {
                Google.Api.Ads.AdManager.v202505.Date date =
                    (Google.Api.Ads.AdManager.v202505.Date) value;
                return string.Format("{0:0000}-{1:00}-{2:00}", date.year, date.month, date.day);
            }
            else if (value is Google.Api.Ads.AdManager.v202505.DateTime)
            {
                Google.Api.Ads.AdManager.v202505.DateTime dateTime =
                    (Google.Api.Ads.AdManager.v202505.DateTime) value;
