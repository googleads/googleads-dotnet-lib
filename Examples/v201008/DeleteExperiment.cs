...
namespace com.google.api.adwords.examples.v201008 {
  ...
  /// GetAllExperiments.cs. To add an experiment, run AddExperiment.cs.
  ///
  /// Tags: ExperimentService.mutate
  /// </summary>
  class DeleteExperiment : SampleBase {
    /// <summary>
    /// Returns a description about the code example.
    /// </summary>
    public override string Description {
      get {
        return "This example deletes an experiment. To get experiments, run GetAllExperiments.cs." +
            "To add an experiment, run AddExperiment.cs.";
      }
    }
    ...
    public override void Run(AdWordsUser user) {
      ...
      experiment.status = ExperimentStatus.DELETED;
      ...
      try {
        ...
        if (retVal != null && retVal.value != null) {
          foreach (Experiment tempExperiment in retVal.value) {
            Console.WriteLine("Experiment with name = \"{0}\" and id = \"{1}\" was deleted.\n",
                tempExperiment.name, tempExperiment.id);
          }
        } else {
          Console.WriteLine("No experiments were deleted.\n");
        }
      } catch (Exception ex) {
        Console.WriteLine("Failed to delete experiment(s). Exception says \"{0}\"", ex.Message);
      }
    }
  }
}
